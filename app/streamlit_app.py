...
try:
    ...
    # Patch MT19937.__setstate__ to handle newer state formats
    _original_mt_setstate = MT19937.__setstate__

    def _patched_mt_setstate(self, state):
        try:
            _original_mt_setstate(self, state)
        except ValueError:
            # Convert newer state format to legacy format
            if isinstance(state, dict) and "bit_generator" in state:
                s = state.get("state", {})
                key = s.get("key", np.zeros(624, dtype=np.uint32))
                pos = s.get("pos", 624)
                legacy_state = {
    ...
    MT19937.__setstate__ = _patched_mt_setstate
except Exception:
    pass 
